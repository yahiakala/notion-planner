...
def rebalance1(notion_token, database_id, property_mappings, defaults, max_hrs=6):
    ...
    taskfilter = {
        'property': property_mappings['done'],
    ...
    sorts = [
        {
            "property": property_mappings['hard_deadline'],
            "direction": "descending"
        },
        {
            "property": property_mappings['deadline'],
            "direction": "ascending"
        }
    ]
    input_data = read.get_all_data(
        notion_token, database_id,
        property_mappings, defaults, queryfilter=taskfilter, sorts=sorts
    )
    ...
    for task, task_old in zip(rescheduled, input_data):
        ...
        if first_before_deadline: # Fit this task into your schedule!
        ...
        elif task['hard_deadline'] and lightest_before_deadline:  # Squeeze in hard deadline
            task['scheduled'] = lightest_before_deadline
        elif task['hard_deadline'] and task['deadline']['datetime'] > days_df['day'].max():
            ...
            task['scheduled'] = {'datetime': None, 'string': ''}
            task['deadline'] = {'datetime': days_df.iloc[-1, 0], 'string': days_df.index[-1]}
        ...
        if task['scheduled'] != task_old['scheduled'] or task['deadline'] != task_old['deadline']:
            task['changed'] = True
        else:
            task['changed'] = False
...
def get_first_day_available(task, availability):
    ...
    if len(first_day) > 0:
        return {
            'datetime': first_day.iloc[0, 0],
            'string': first_day.index[0]
        }
    else:
        return None
...
def get_lightest_day_available(task, availability):
    ...
    """
    lightest_day = (
        availability[availability['day'] < task['deadline']['datetime']]
        .sort_values(by=['curr_load', 'day'])
    )
    if len(lightest_day) > 0:
        return {
            'datetime': lightest_day.iloc[0, 0],
            'string': lightest_day.index[0]
        }
    return None
...
def get_first_day_after(task, availability):
    ...
    """
    first_day = (
        availability[availability['curr_avail'] >= task['hours']]
        .sort_values(by=['day'])
    )
    if len(first_day) > 0:
        return {
...
def get_days_df(curr_load={'hours': 0}, days_ahead=7, max_hrs=6):
    ...
    days_df['curr_load'].fillna(0., inplace=True)
...
def rebalance0(notion_token, database_id, property_mappings, defaults, max_hrs=6):
    ...
    input_data0 = read.get_all_data(
        notion_token, database_id,
        property_mappings, defaults, queryfilter=taskfilter
    )
    print(input_data0[:2])
    ...
    input_data.sort_values(by=['hard_deadline', 'deadline'],
    ...
    rescheduled.loc[
        rescheduled['scheduled'] == today2, 'scheduled_str'] = today2_str
    days_df = days_dfi.copy()

    for ind, row in rescheduled.iterrows():
        # print(row['title'])
        if (all(days_df['curr_avail'] < row['hours']) and
            row['deadline'] > days_df['day'].max()):
            ...
            day_assignment_2 = (
                days_df[days_df['day'] < row['deadline']]
                .sort_values(by=['curr_load', 'day'])
            )
            # Third, find the first available day after deadline
            day_assignment_3 = (
                days_df[days_df['curr_avail'] >= row['hours']]
                .sort_values(by=['day'])
            )
            ...
            if len(day_assignment_1) > 0:
                ...
                # print('--There is time')
            elif len(day_assignment_3) > 0 and not rescheduled.loc[ind, 'hard_deadline']:
