...
def patch_page(notion_token, page_id, payload_dict):
    ...
    response = anvil.http.request(url, method='PATCH', data=payload_dict, json=True, headers=headers)
    return response
...
def prop_date_value(notion_token, page_id, prop_name, date_val, verbose=True):
    ...
    date_obj = {
        'date': {'start': date_val, 'end': None, 'time_zone': None}
    }
    if date_val == '' or date_val == None:  # noqa
        date_obj = {'date': None}
    payload = {
        "properties": {prop_name: date_obj}
    }
