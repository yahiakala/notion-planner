    ...
    if property_object["date"]:
        return {
            ...
            "datetime": dt.datetime.strptime(
                property_object["date"]["start"], "%Y-%m-%d"
            ),
        }
    else:
        return {
            "string": "",
            "datetime": dt.datetime.strptime("2099-01-01", "%Y-%m-%d"),
        }
...
def search_databases(notion_token):
    ...
    response = anvil.http.request(
        url, method="POST", data=payload, json=True, headers=headers
    )
    return response


def get_response(database_id, notion_token, queryfilter=None, sorts=None, starter=None):
    """
...
def format_response(data, prop_mapping, defaults=None):
    ...
    for i in data["results"]:
        ...
        for property in prop_types:
            task_prop[property["name"]] = prop_proc_dict[property["type"]](
                task_prop[property["name"]]
            )
...
def get_all_data(
    ...
    data_list = []
    resp1 = get_response(
        database_id, notion_token, queryfilter=queryfilter, sorts=sorts
    )
